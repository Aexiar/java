...
* ① 面试的需求，如：BATJ、TMD 等面试都爱问。
* ② 中高级程序员必备的技能，如：项目管理、调优的需要。
* ③ 追求极客的精神，如：垃圾回收算法、JIT 以及底层原理。
...
* `Java 虚拟机就是二进制字节码的运行环境`：负责装载字节码到其内部，解释或编译为对应平台上的机器指令并执行。每一条 Java 指令，JVM 虚拟机规范中都有详细的定义，如：怎么取操作数、怎么处理操作数、处理的结果放在哪里。
