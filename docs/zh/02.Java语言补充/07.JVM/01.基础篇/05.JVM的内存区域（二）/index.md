...
* 对于 CPU 来说，整个世界真的太慢了；其实，中国古代的文人也有很多作品来表示这种速度上的对比：
...
* 其实，对于虚拟机来说，有两种架构，如下所示：
  * :one: 基于`栈`架构的虚拟机，而栈是位于内存中，速度较慢。
  * :two: 基于`寄存器`架构的虚拟机，而寄存器是位于 CPU 内部，速度较快。
...
> [!NOTE]
>
> * ① 这种架构的优点是可以`跨平台`，指令集小（8 位），编译器容易实现。
...
> [!NOTE]
>
> * ① 这种架构的优点是可以`性能高`，指令集大（往往以一地址指令、二进制指令和三地址指令为主）。
...
> [!NOTE]
>
> 由于`方法`可能会在不同的线程中执行，每个线程都会包含自己独立的`虚拟机栈`。
